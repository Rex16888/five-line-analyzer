...
# === 檔案上傳 ===
st.subheader("📤 匯入股票代碼清單")
uploaded_file = st.file_uploader("請上傳 CSV 或 Excel（每列一筆代碼）", type=["csv", "xlsx"])

# === 手動輸入 ===
st.subheader("✍️ 手動輸入股票代碼")
manual_input = st.text_input("多筆代碼請用逗號分隔（如 AAPL, TSLA, MSFT）")

# === 區間選擇 ===
st.subheader("📆 選擇分析時間區間")
date_range = st.selectbox("請選擇抓取歷史股價的區間：", ["1y", "2y", "3y", "5y", "10y"], index=3)
